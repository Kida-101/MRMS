...
const Header = () => {
  ...
  return (
    <div className="sticky top-0 z-50 flex justify-between w-full px-4 py-3 border-b border-dashed items-center backdrop-blur-md bg-background/60">
      ...
      <div className="flex items-center gap-4">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-xs">
            <DropdownMenuLabel>Ab Marshal</DropdownMenuLabel>
            {/* From the backend */}
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem>
              <CircleUserRound size={16} /> <Link href="#">Edit profile</Link>
            </DropdownMenuItem>
            <DropdownMenuItem>
              <Settings size={16} /> <Link href="#">Account settings</Link>
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        {/* Notification */}
            ...
            {maintainanceReqs.map((mreq) => (
              <DropdownMenuItem className="w-xs flex items-center mt-5">
                <Link href="#" className="flex gap-3 items-center">
                  <Avatar>
                    <AvatarImage src={mreq.image} />
                    {/* From the backend */}
                    <AvatarFallback>CN</AvatarFallback>
                  </Avatar>
                  <div className="">
                    <div>
                      {mreq.name} <span>{mreq.requestDescription}</span>
                    </div>
