...
export const deleteRoom = async (req, res) => {
  ...
  try {
    ...
    if (room.tenantId) {
      return res.status(400).json({ success: false, message: "Room has a tenant, can't be deleted" });
    }

    // delete the room
    const deletedRoom = await Room.findByIdAndDelete(id);
    if (!deletedRoom) {
      return res.status(400).json({ success: false, message: "Failed to delete a room" });
    }
